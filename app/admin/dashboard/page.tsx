...
import { ArrowLeft, ArrowRight, LayoutDashboard, TrendingDown, TrendingUp } from 'lucide-react';
...
async function loadMonthMinutes(
  month: string,
  status: string
): Promise<{ rows: Row[]; totalMinutes: number }> {
...
function StatTile({ label, value, delta }: { label: string; value: string; delta?: React.ReactNode }) {
  return (
    <div className="card p-4 sm:p-5">
      <p className="text-xs sm:text-sm text-[color:var(--color-ink-secondary)]">{label}</p>
...
export default async function AdminDashboardPage({
  ...
  const peopleMinutes = new Map<string, { nama: string; minutes: number }>();
  for (const r of rows) {
    ...
    peopleMinutes.set(key, { nama: r.nama, minutes: (existing?.minutes ?? 0) + minutes });
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="card p-3 flex items-center justify-center gap-4">
        <Link href={monthHref(prevMonth)} className="btn-secondary px-3">
          <ArrowLeft size={15} /> Bulan Lalu
        </Link>
        <p className="text-base font-semibold min-w-[150px] sm:min-w-[180px] text-center">
          {formatMonthLabelID(month)}
        </p>
        <Link href={monthHref(nextMonth)} className="btn-secondary px-3">
          Bulan Depan <ArrowRight size={15} />
        </Link>
      </div>
            ...
            {ranking.map((p) => {
              ...
              return (
                <div key={p.nama} className="flex items-center gap-3">
                  <p className="w-24 sm:w-40 shrink-0 truncate text-sm">{p.nama}</p>
                  <div
                  ...
                  <p className="w-16 sm:w-24 shrink-0 text-right text-sm tabular-nums text-[color:var(--color-ink-secondary)]">
      ...
      <div className="card p-4 sm:p-6">
        ...
        {totalMinutes === 0 ? (
          ...
          <div className="overflow-x-auto">
            <div
              className="flex items-end gap-[2px] h-40 border-b border-[color:var(--color-border)]"
